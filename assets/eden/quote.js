function initEdenQuote(){
 ...
 function reset(){window.edenManual=false;el('bookSubmit').textContent='Réserver et passer au paiement →';showDetails(false);sequence++;window.edenQuote=null;el('quotePrice').textContent='';el('quoteStatus').textContent='Calculez le prix de votre trajet.';el('routeMap').hidden=true;}
 ...
 const q=await api({action:'quote',pickup,dropoff,pickupAt,service});if(current!==sequence)return;window.edenManual=false;window.edenQuote=q;el('bookSubmit').textContent='Réserver et passer au paiement →';el('submitNote').textContent='Paiement sécurisé par SumUp. Votre bon est disponible après réservation.';showDetails(true);el('quotePrice').textContent=money(q.amount);el('quoteStatus').textContent=`${(q.distanceMeters/1000).toLocaleString('fr-FR',{maximumFractionDigits:2})} km · environ ${Math.ceil(q.durationSeconds/60)} min · 1,70 €/km TTC`;
