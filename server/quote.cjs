...
async function quote(input){
 ...
 const r=await fetch('https://routes.googleapis.com/directions/v2:computeRoutes',{method:'POST',headers:{'Content-Type':'application/json','X-Goog-Api-Key':process.env.GOOGLE_ROUTES_API_KEY,'X-Goog-FieldMask':'routes.distanceMeters,routes.duration,routes.polyline.encodedPolyline'},body:JSON.stringify({origin:{address:pickup},destination:{address:dropoff},travelMode:'DRIVE',routingPreference:'TRAFFIC_UNAWARE',languageCode:'fr-FR',units:'METRIC',routeModifiers:{avoidFerries:true}}),signal:AbortSignal.timeout(10000)});
 ...
 const q={id:randomUUID(),pickup,dropoff,pickupAt:input.pickupAt,service:input.service,distanceMeters:route.distanceMeters,durationSeconds:Math.ceil(parseFloat(route.duration)),amount:Math.round(route.distanceMeters*170/1000),expires:Date.now()+15*60000};
